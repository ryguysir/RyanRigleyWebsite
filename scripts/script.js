...
allTags.forEach((e) => {
  e.addEventListener("click", (event) => {
    ...
    for (
      ...
      if (
        document
          ...
          [i].classList.contains("tagToggled")
      ) {
        document
          .getElementById("loopingLottie")
    ...
    if (
      event.srcElement.innerHTML == "Browse All"
    ) {
      if (
    ...
    if (
      document
        .getElementsByClassName("a2z")[0]
        .classList.contains("tagToggled")
    ) {
      removeAllChildNodes(
        document.getElementById(
          "videoBoxContainer"
      ...
      document
        .getElementsByClassName("a2z")[0]
        .classList.toggle("tagToggled");
    }
    if (
      allTagsSearchList.includes(
        event.srcElement.innerHTML
      )
    ) {
      allTagsSearchList.splice(
        allTagsSearchList.indexOf(
          event.srcElement.innerHTML
        ),
        1
      );
    } else {
      allTagsSearchList.push(
        event.srcElement.innerHTML
      );
    }
...
function VideoBox() {
  ...
  this.create = function createVideoBox(videos) {
    ...
    let videoBoxImageContainer = document.createElement(
      "div"
    );
    let videoImage = document.createElement(
      "img"
    );
    ...
    videoBoxImageContainer.setAttribute(
      "class",
      "videoBoxImageContainer"
    );
    ...
    let descriptionElem = document.createElement(
      "p"
    );
    ...
    let lottie = document.createElement(
      "lottie-player"
    );
    if (!isMobile) {
      lottie.loop = false;
      ...
      lottie.src =
        "https://assets6.lottiefiles.com/packages/lf20_fqg8yuqr.json";
    ...
    document
      .getElementById("videoBoxContainer")
    ...
    lottie.addEventListener("click", () => {
      ...
      let xCloseButton = document.createElement(
        "lottie-player"
      );
      xCloseButton.loop = false;
      xCloseButton.autoplay = true;
      xCloseButton.src =
        "https://assets9.lottiefiles.com/packages/lf20_smbgcrqi.json";
      xCloseButton.controls = false;
      xCloseButton.speed = 0.5;
      xCloseButton.setAttribute(
        "class",
        "xCloseButton"
      );
...
(function createVideos() {
  ...
  johnHenry.imgSrc =
    "images/johnHenryAndTheStatesmen.jpg";
  ...
  peoplePlacesThings.imgSrc =
    "images/peoplePlacesThings.jpg";
  ...
  aVeryMurrayChristmas.videos = [
    "209141658",
    "209141623",
    "209141581",
  ];
  aVeryMurrayChristmas.title =
  ...
  aVeryMurrayChristmas.imgSrc =
    "images/aVeryMurrayChristmas.jpg";
...
function rankSort() {
  videoBoxStorage.sort((a, b) => {
    return (
      b.rank +
      b.rankImportance -
      (a.rank + a.rankImportance)
    );
  });
  for (i = 0; i < videoBoxStorage.length; i++) {
    if (videoBoxStorage[i].rank >= 1) {
      videoBoxStorage[i].create(
        videoBoxStorage[i].videos
      );
    }
  }
}
...
function searchBox() {
  //set all ranks to 0
  videoBoxStorage.forEach(
    (element) => (element.rank = 0)
  );
  //remove all video boxes from container
  removeAllChildNodes(
    document.getElementById("videoBoxContainer")
  );

  let searchQuery = document.getElementById(
    "searchBox"
  ).value;
  ...
  for (i = 0; i < videoBoxStorage.length; i++) {
    for (
      j = 0;
      j < videoBoxStorage[i].tags.length;
      j++
    ) {
      if (
        videoBoxStorage[i].tags[j] ==
...
function browseAll() {
  ...
  for (
    i = 0;
    i <
    document.getElementsByClassName("tag").length;
    i++
  ) {
    if (
      document
  ...
  function sortTitles() {
    return new Promise((resolve) => {
      ...
      let sorted = [...videoBoxStorage].sort(
        (a, b) => {
          return (
            alpha.indexOf(
              a.title.toLowerCase()[0]
            ) -
            alpha.indexOf(
              b.title.toLowerCase()[0]
            )
          );
        }
      );
...
function tagSearch() {
  ...
  for (i = 0; i < videoBoxStorage.length; i++) {
    for (
      j = 0;
      j < allTagsSearchList.length;
      j++
    ) {
      if (
        videoBoxStorage[i].tags.includes(
          allTagsSearchList[j].toLowerCase()
        )
      ) {
        videoBoxStorage[i].rank += 1;
      }
